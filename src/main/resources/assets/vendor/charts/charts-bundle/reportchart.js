...
$(function() {
    ...
    var ctxWeeklyReports = document.getElementById('weeklyReportsChart').getContext('2d');
    var weeklyReportsChart = new Chart(ctxWeeklyReports, {
        type: "line",
        data: {
    ...
    // Pie Chart for Account Categories
    if ($('#accountPieChart').length) {
        var ctxPieChart = document.getElementById('accountPieChart').getContext('2d');
        var data = {
            labels: ['스팸 및 광고', '선정적인 게시물', '개인정보노출', '영리목적/홍보성', '개인정보 노출'],
            datasets: [{
                data: [2.50, 16.69, 6.34, 23.03, 51.44],
                backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#555555']
            }]
        };
                ...
                tooltip: {
                    callbacks: {
                        label: function(tooltipItem, data) {
                            ...
                            label += data.datasets[tooltipItem.datasetIndex].data[tooltipItem.dataIndex] + '건';
                            return label;
                        }
                    }
                }
